...
var mod = { $pushAll: { moves: ['chute no saco','fura zóio']}};
var op = { multi: true };
db.pokemons.update(query, mod, op);

Updated 3 existing record(s) in 230ms
WriteResult({
  "nMatched": 3,
  "nUpserted": 0,
  "nModified": 3
...
Updated 610 existing record(s) in 28ms
WriteResult({
  "nMatched": 610,
  "nUpserted": 0,
  "nModified": 610
...
Updated 1 new record(s) in 56ms
WriteResult({
  "nMatched": 0,
  "nUpserted": 1,
  "nModified": 0,
  "_id": ObjectId("565ced2cb1a50f975cd8cd5c")
})


```
## 4. Pesquisar todos o pokemons que possuam o ataque `investida` e mais um que você adicionou, e escolha seu pokemon favorito.
```
var query = {moves: {$in: ['investida', /chute no saco/i]}}
...
```
var query = {moves: {$all: ['fura zóio', /chute no saco/i]}}
db.pokemons.find(query)
...
Fetched 20 record(s) in 56ms -- More[true]
...
## 8. Esse item não está no vídeo e se você fizer significa que você lê as coisas, nesse exercício demonstre qual a diferença entre os operadores `$ne` e `$not`.


O operador `$ne` significa NOT EQUALS. Ou popularmente chamado de DIFERENTE. Este não aceita o uso de REGEX.

O operador `$not` é um Operador Lógico de Negação e funciona com regex.
