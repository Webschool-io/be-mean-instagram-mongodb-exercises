...
```
localhost(mongod-3.2.1) be-mean> db.pokemons.find({"name": "Magneton"}).explain('executionStats')
{
  "queryPlanner": {
    "plannerVersion": 1,
...
```
localhost(mongod-3.2.1) be-mean> var query = {$and: [{name: "Magneton"},{speed: 70}]}
localhost(mongod-3.2.1) be-mean> db.pokemons.find(query).explain('executionStats')
{
  "queryPlanner": {
    "plannerVersion": 1,
    "namespace": "be-mean.pokemons",
...
```
localhost(mongod-3.2.1) be-mean> db.pokemons.createIndex({name: 1, speed: 1})
{
