...
mongoimport --db be-mean --collection pokemons --drop --file C:\Users\Triforce\Study\be-mean-modulo-mongodb\json-examples\pokemons.json
2015-12-12T15:27:39.484-0200    connected to: localhost
2015-12-12T15:27:39.486-0200    dropping: be-mean.pokemons
2015-12-12T15:27:39.523-0200    imported 610 documents
...
db.pokemons.group({
  ...
  reduce: function(curr, result) {
    curr.types.forEach(function(property) {
      if(result[property]){ result[property]++ } else { result[property] =  1 }
      result.count++;
    })
  }
})
...
> db.pokemons.count({speed:{$lte : 50}})
218

> db.pokemons.count({ hp : { $gte :  30 }})
600

> db.pokemons.count({ hp : { $gte :  200 }})
2
```
