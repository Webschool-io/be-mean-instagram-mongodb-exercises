...
```
	MacBook-de-Carlos:~ carlos$ mongoimport --host 127.0.0.1 --db be-mean --collection restaurantes --drop --file ~/Downloads/restaurantes.json 
	...
	MacBook-de-Carlos:~ carlos$ mongoimport --host 127.0.0.1 --db be-mean --collection pokemons --drop --file ~/Downloads/pokemons.json 
...
```
	MacBook-de-Carlos(mongod-3.0.5) be-mean> db.restaurantes.distinct('cuisine').sort()
	[
	  "Afghan",
	  "African",
	  "American ",
...
```
	MacBook-de-Carlos(mongod-3.0.5) be-mean> db.pokemons.distinct('types').sort()
	[
	  "bug",
	  "dark",
	  "dragon",
...
```
	MacBook-de-Carlos(mongod-3.0.5) be-mean> db.pokemons.find({}, {name: 1, _id: 0}).limit(5).skip(5 * 0)
	{
	  "name": "Charmander"
	}
	{
	...
	MacBook-de-Carlos(mongod-3.0.5) be-mean> db.pokemons.find({}, {name: 1, _id: 0}).limit(5).skip(5 * 1)
	{
	  "name": "Metapod"
	}
	{
	...
	Fetched 5 record(s) in 1ms
	MacBook-de-Carlos(mongod-3.0.5) be-mean> db.pokemons.find({}, {name: 1, _id: 0}).limit(5).skip(5 * 2)
	{
	  "name": "Magnemite"
	}
	{
...
```
	MacBook-de-Carlos(mongod-3.0.5) be-mean> db.pokemons.aggregate([
	...     { 
	...     $unwind: "$types" 
	...     },
	...     { 
